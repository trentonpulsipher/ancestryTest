...
Generating insights may be as simple as some in-depth visualization, but modeling the data will provide more definitive value. More modeling, I recommend subsetting to relevant data (only customers that had `ordercreatedate`s between mid 2013 to mid 2016) and prediction the percent of cross-sell conversions. I wish I understood a little more about these other variables like `regtenure`, `dna_visittrafficsubtype`, `xsell_day_exact`, and even `customer_type_grp`. A deeper business awareness might help me know how to leverage those variables when modeling.

Models we could use include any regression based methods or machine learning (ML) algorithms. ML is primarily beneficial when predicting, but we could choose one of the few algorithms where people have built explainer functions. Our predictive capability may be great, but we would likely lack the insight needed to approach future experimentation/adjustments to drive the conversion higher.
...
2. return to the experimentation to try to find additional support/evidence in a way not previously experimented upon, or 
